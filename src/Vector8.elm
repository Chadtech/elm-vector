...
{-| Try and turn an `Int` into an `Index`, returning `Nothing` if the `Int` is above the maximum index of this `Vector8 a` -}
...
{-| Make a `Vector8 a` from 8elements -}
...
push : a -> Vector8 a -> Vector9.Vector a
...
pop : Vector8 a -> ( Vector7.Vector a, a )
pop (Vector vector) =
    (
    { n0 = vector.n0
    , n1 = vector.n1
...
uncons : Vector8 a -> ( a, Vector7.Vector a )
...
cons : a -> Vector8 a -> Vector9.Vector a
