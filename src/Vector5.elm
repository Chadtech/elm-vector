module Vector5 exposing
    ( Vector5
    , fromList, repeat, from5, fromListWithDefault, initializeFromInt, initializeFromIndex
...
import Vector5.Internal exposing (Vector5(..), VectorModel)
...
{-| a `Vector5 a` using a function that takes an `Int`, representing the index
-}
...
{-| a `Vector5 a` filled with just one item repeated over and over again.
-}
repeat : a -> Vector5 a
repeat a =
    { n0 = a
...
{-| Try and turn an `Int` into an `Index`, returning `Nothing` if the `Int` is above the maximum index, or below the zero index, of this `Vector5 a`
...
{-| A list of all the indices, from 0 to 4
-}
indices : Vector5 Index
indices =
...
-}
push : a -> Vector5 a -> Vector6.Vector6 a
...
-}
pop : Vector5 a -> ( Vector4.Vector4 a, a )
pop (Vector vector) =
    ( { n0 = vector.n0
      , n1 = vector.n1
      , n2 = vector.n2
...
-}
uncons : Vector5 a -> ( a, Vector4.Vector4 a )
...
-}
cons : a -> Vector5 a -> Vector6.Vector6 a
