module Vector3 exposing
    ( Vector3
    , fromList, repeat, from3, fromListWithDefault, initializeFromInt, initializeFromIndex
...
{-| a `Vector3 a` using a function that takes an `Int`, representing the index
-}
...
{-| a `Vector3 a` filled with just one item repeated over and over again.
-}
repeat : a -> Vector3 a
repeat a =
    { n0 = a
