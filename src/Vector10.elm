module Vector10 exposing
    ( Vector10
    , fromList, repeat, from10, fromListWithDefault, initializeFromInt, initializeFromIndex
...
import Vector10.Internal exposing (Vector10(..), VectorModel)
...
{-| a `Vector10 a` using a function that takes an `Int`, representing the index
-}
...
{-| a `Vector10 a` filled with just one item repeated over and over again.
-}
repeat : a -> Vector10 a
repeat a =
    { n0 = a
...
{-| Try and turn an `Int` into an `Index`, returning `Nothing` if the `Int` is above the maximum index, or below the zero index, of this `Vector10 a`
...
{-| A list of all the indices, from 0 to 9
-}
indices : Vector10 Index
indices =
...
-}
push : a -> Vector10 a -> Vector11.Vector11 a
...
-}
pop : Vector10 a -> ( Vector9.Vector9 a, a )
pop (Vector vector) =
    ( { n0 = vector.n0
      , n1 = vector.n1
      , n2 = vector.n2
...
-}
uncons : Vector10 a -> ( a, Vector9.Vector9 a )
...
-}
cons : a -> Vector10 a -> Vector11.Vector11 a
