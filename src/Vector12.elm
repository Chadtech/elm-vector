...
{-| Make a `Vector12 a` using a function that takes an `Int`, representing the index -}
...
{-| Make a `Vector12 a` filled with just one item repeated over and over again. -}
repeat : a -> Vector12 a
repeat a =
    { n0 = a
    , n1 = a
...
{-| Try and turn an `Int` into an `Index`, returning `Nothing` if the `Int` is above the maximum index of this `Vector12 a` -}
...
{-| Make a `Vector12 a` from 12elements -}
...
pop : Vector12 a -> ( Vector11.Vector a, a )
pop (Vector vector) =
    (
    { n0 = vector.n0
    , n1 = vector.n1
