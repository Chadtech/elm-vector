module Vector2 exposing
    ( Vector2
    , fromList, repeat, from2, fromListWithDefault, initializeFromInt, initializeFromIndex
...
{-| a `Vector2 a` using a function that takes an `Int`, representing the index
-}
...
{-| a `Vector2 a` filled with just one item repeated over and over again.
-}
repeat : a -> Vector2 a
repeat a =
    { n0 = a
