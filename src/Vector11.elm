module Vector11 exposing
    ( Vector11
    , fromList, repeat, from11, fromListWithDefault, initializeFromInt, initializeFromIndex
...
import Vector11.Internal exposing (Vector11(..), VectorModel)
import Vector12.Internal as Vector12


{-| A vector that contains exactly 11 elements
-}
type alias Vector11 a =
    Vector11.Internal.Vector11 a


{-| All the indices to a `Vector11 a`. There are exactly 11 of them. Its kind of like an `Int` except there is a finite amount of them.
-}
type Index
    = Index0
    | Index1
...
{-| Get the item at that `Index` in a `Vector11 a`
-}
...
{-| Set the item at a specific index in a `Vector11 a`
-}
set : Index -> a -> Vector11 a -> Vector11 a
...
{-| Reduce a `Vector11 a` from the right
-}
foldr : (a -> b -> b) -> b -> Vector11 a -> b
foldr f start vector =
    List.foldr f start <| toList vector


{-| Reduce a `Vector11 a` from the left
-}
foldl : (a -> b -> b) -> b -> Vector11 a -> b
foldl f start vector =
    List.foldl f start <| toList vector


{-| Apply a function to every element in a \`Vector11 a
-}
...
-}
indexedMap : (Index -> a -> b) -> Vector11 a -> Vector11 b
...
{-| -}
map2 : (a -> b -> c) -> Vector11 a -> Vector11 b -> Vector11 c
...
{-| -}
map3 : (a -> b -> c -> d) -> Vector11 a -> Vector11 b -> Vector11 c -> Vector11 d
...
{-| -}
map4 : (a -> b -> c -> d -> e) -> Vector11 a -> Vector11 b -> Vector11 c -> Vector11 d -> Vector11 e
...
{-| -}
map5 : (a -> b -> c -> d -> e -> f) -> Vector11 a -> Vector11 b -> Vector11 c -> Vector11 d -> Vector11 e -> Vector11 f
...
{-| Convert a `Vector11 a` into a `List a` of length 11
-}
...
{-| Turn a `List a` into a `Vector11 a`. If there are not enough items in the `List a`, then this function returns `Nothing`. The extra items in the input list, if there are any, is returned as the first element in the output tuple.
...
{-| Turn a `List a` into a `Vector11 a`. If there are not enough items in the `List a`, then fill in the remaining spots with a default value. The extra items in the input list, if there are any, is returned as the first element in the output tuple.
...
-}
fromListWithDefault : a -> List a -> ( List a, Vector11 a )
...
{-| Turn a `Vector11 a` elm into a list, where each element is paired with its `Index`
-}
...
{-| a `Vector11 a` using a function that takes an `Int`, representing the index
-}
...
{-| Make a `Vector11 a` using a function that takes an `Index`
-}
...
{-| a `Vector11 a` filled with just one item repeated over and over again.
-}
repeat : a -> Vector11 a
repeat a =
    { n0 = a
...
{-| Try and turn an `Int` into an `Index`, returning `Nothing` if the `Int` is above the maximum index, or below the zero index, of this `Vector11 a`
...
{-| a `Vector11 a` from 11 elements
-}
from11 : a -> a -> a -> a -> a -> a -> a -> a -> a -> a -> a -> Vector11 a
from11 a0 a1 a2 a3 a4 a5 a6 a7 a8 a9 a10 =
    { n0 = a0
    , n1 = a1
    , n2 = a2
    , n3 = a3
...
{-| See if a Vector11 a contains a value
-}
member : a -> Vector11 a -> Bool
...
{-| Reverse the order of the items in a `Vector11 a`
-}
reverse : Vector11 a -> Vector11 a
...
{-| The length of this vector type, which is 11
-}
length : Int
length =
    11
...
{-| Break a `List a` down into groups of `Vector11 a`. The output is a tuple, where the left side is a list of the remainder.
...
-}
group : List a -> ( List a, List (Vector11 a) )
group items =
    groupHelp items []


groupHelp : List a -> List (Vector11 a) -> ( List a, List (Vector11 a) )
groupHelp remainingItems output =
    case remainingItems of
        i0 :: i1 :: i2 :: i3 :: i4 :: i5 :: i6 :: i7 :: i8 :: i9 :: i10 :: rest ->
            groupHelp rest (from11 i0 i1 i2 i3 i4 i5 i6 i7 i8 i9 i10 :: output)

        _ ->
            ( remainingItems, List.reverse output )


{-| A list of all the indices, from 0 to 10
-}
indices : Vector11 Index
indices =
    initializeFromIndex identity


{-| Add an element to the end of a `Vector11 a`, incrementing its size by 1

    Vector4.push 4 (Vector4.from4 0 1 2 3)
    --> Vector5.from5 0 1 2 3 4

-}
push : a -> Vector11 a -> Vector12.Vector12 a
...
{-| Separate a `Vector11 a` into its last element and everything else.

    Vector4.pop (Vector4.from4 0 1 2 3)
    --> (Vector3.from3 0 1 2, 3)

-}
pop : Vector11 a -> ( Vector10.Vector10 a, a )
pop (Vector vector) =
    ( { n0 = vector.n0
      , n1 = vector.n1
      , n2 = vector.n2
...
{-| Split a `Vector11 a` into its first element and the rest

    Vector4.uncons (Vector4.from4 0 1 2 3)
    --> (0, Vector3.from3 1 2 3)

-}
uncons : Vector11 a -> ( a, Vector10.Vector10 a )
...
-}
cons : a -> Vector11 a -> Vector12.Vector12 a
